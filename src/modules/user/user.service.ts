...
export class UserService {
  ...
  async getAllUsers(
    ...
    page: number = 1,
    limit: number = 10,
  ): Promise<Pagination<Omit<UserEntity, 'password'>>> {
    ...
    const cached =
      await this.redisCacheService.get<Pagination<Omit<UserEntity, 'password'>>>(
        cacheKey,
      );
    ...
    const result = new Pagination<Omit<UserEntity, 'password'>>({
      results,
      total,
